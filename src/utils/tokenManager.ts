...
class TokenManager {
  ...
  // Fetch token from ServiceNow endpoint or use g_ck
  private async fetchTokenFromEndpoint(): Promise<string> {
    const config = getEnvironmentConfig();

    // First try to get token from ServiceNow g_ck global
    if (typeof window !== 'undefined') {
      // @ts-expect-error - ServiceNow globals
      const gck = window.g_ck;
      if (gck) {
        envLog.info('Using g_ck token from ServiceNow globals');
        
        this.cachedToken = {
          token: gck,
          fetchedAt: Date.now(),
          expiresAt: undefined // g_ck doesn't have explicit expiry
        };
    ...
    // Fallback to token endpoint if no g_ck available
    ...
    try {
      ...
      envLog.info('Token response received');
