...
export interface LayoutAlgorithm {
    ...
    calculate: (treeData: TreeNodeData, dimensions: LayoutDimensions, performanceMode: "auto" | "high" | "maximum", radialSettings?: RadialLayoutSettings) => LayoutResult;
}
...
abstract class BaseLayout implements LayoutAlgorithm {
    ...
    abstract calculate(treeData: TreeNodeData, dimensions: LayoutDimensions, performanceMode: "auto" | "high" | "maximum", radialSettings?: RadialLayoutSettings): LayoutResult;
}
...
export class HorizontalTreeLayout extends BaseLayout {
    ...
    calculate(treeData: TreeNodeData, dimensions: LayoutDimensions, performanceMode: "auto" | "high" | "maximum", radialSettings?: RadialLayoutSettings): LayoutResult {
        ...
        const maxNodesAtLevel = Math.max(...Array.from({ length: depth + 1 }, (_, i) => root.descendants().filter((d) => d.depth === i).length));

        // Dynamic sizing with increased parent-child spacing
        let layoutWidth = Math.max(availableWidth, depth * 300); // Set to 1500 for optimal spacing
