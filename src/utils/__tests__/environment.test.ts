...
describe('Environment Utils', () => {
  beforeEach(() => {
    ...
    delete (global as Record<string, unknown>).g_ck;
    delete (global as Record<string, unknown>).NOW;
  });
  ...
  describe('isServiceNowEnvironment', () => {
    ...
    it('returns true when g_ck global is present', () => {
      (global as Record<string, unknown>).g_ck = 'test-session-token';
      expect(isServiceNowEnvironment()).toBe(true);
    });

    it('returns true when NOW global is present', () => {
      (global as Record<string, unknown>).NOW = { user: { userID: 'test' } };
      expect(isServiceNowEnvironment()).toBe(true);
    });

    it('returns true when window.g_ck is present', () => {
      (global as Record<string, unknown>).window = { g_ck: 'test-session-token' };
      expect(isServiceNowEnvironment()).toBe(true);
    });
    ...
    it('caches the result', () => {
      ...
      (global as Record<string, unknown>).g_ck = 'test-token';
  ...
  describe('getServiceNowGlobals', () => {
    ...
    it('returns NOW object when present', () => {
      ...
      (global as Record<string, unknown>).NOW = nowObject;
      const globals = getServiceNowGlobals();
      expect(globals.NOW).toEqual(nowObject);
    });

    it('prefers window globals over direct globals', () => {
      (global as Record<string, unknown>).g_ck = 'direct-token';
      (global as Record<string, unknown>).window = { g_ck: 'window-token' };
