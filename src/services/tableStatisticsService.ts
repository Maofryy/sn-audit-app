...
export class TableStatisticsService {
    ...
    async getAllTableStatistics(): Promise<TableStatistics[]> {
        ...
        try {
            ...
            const cmdbTables = TESTING_MODE 
                ? allCmdbTables.filter(table => TEST_TABLES.includes(table.name))
                : allCmdbTables;
            
            if (TESTING_MODE) {
                console.log(`🧪 TESTING MODE: Restricted to ${cmdbTables.length} tables:`, TEST_TABLES);
                console.log('📋 Found tables:', cmdbTables.map(t => t.name));
            } else {
            ...
            const statisticsPromises = cmdbTables.map(table => 
                this.getTableStatistics(table.name)
            );
    ...
    async getFilteredTableList(filters: TableListFilter = {}): Promise<TableListItem[]> {
        try {
            ...
            if (filters.search_term) {
                ...
                tableListItems = tableListItems.filter(item =>
                    item.table.name.toLowerCase().includes(searchLower) ||
                    item.table.label.toLowerCase().includes(searchLower)
                );
            }

            if (filters.table_types?.length) {
                tableListItems = tableListItems.filter(item =>
                    filters.table_types!.includes(item.table.table_type)
                );
            }

            if (filters.audit_status?.length) {
                tableListItems = tableListItems.filter(item =>
                    filters.audit_status!.includes(item.audit_status)
                );
            }
            ...
            if (filters.min_references !== undefined) {
                tableListItems = tableListItems.filter(item =>
                    item.statistics.total_references >= filters.min_references!
                );
            }

            if (filters.max_references !== undefined) {
                tableListItems = tableListItems.filter(item =>
                    item.statistics.total_references <= filters.max_references!
                );
            }
    ...
    private async getTableMetadata(tableName: string): Promise<TableMetadata | null> {
        try {
            const response = await makeAuthenticatedRequest(
                `/api/now/table/sys_db_object?sysparm_query=name=${tableName}&sysparm_limit=1&sysparm_fields=sys_id,name,label`
            );
    ...
    private async getOutgoingReferences(tableName: string): Promise<FieldMetadata[]> {
        const response = await makeAuthenticatedRequest(
            `/api/now/table/sys_dictionary?sysparm_query=table=${tableName}^internal_type=reference&sysparm_limit=1000&sysparm_fields=sys_id,element,column_label,mandatory,reference,sys_created_by`
        );
    ...
    private async getIncomingReferences(tableName: string): Promise<FieldMetadata[]> {
        const response = await makeAuthenticatedRequest(
            `/api/now/table/sys_dictionary?sysparm_query=reference=${tableName}^internal_type=reference&sysparm_limit=1000&sysparm_fields=sys_id,element,table,column_label,mandatory,sys_created_by`
        );
    ...
    private async getFieldUsageStats(tableName: string, fieldName: string): Promise<FieldUsageStats> {
        try {
            // Get total count
            const totalResponse = await makeAuthenticatedRequest(
                `/api/now/stats/${tableName}?sysparm_count=true`
            );
            ...
            // Get populated count
            const populatedResponse = await makeAuthenticatedRequest(
